...
import { isSyncEnabled, sendWebhookSync, getSyncConfig, setSyncConfig, testWebhookConnection } from './hermes-sync.js';
...
// Auto-sync on cookie changes for selected domains
chrome.cookies.onChanged.addListener(async (changeInfo) => {
  if (changeInfo.removed) return; // Skip removals for auto-sync
  
  try {
    const config = await getSyncConfig();
    if (!config.enabled || !config.autoSync) return;
    if (!config.webhookUrl || !config.apiKey) return;

    // Get the cookie that changed
    ...
    // Check if this cookie's domain is in our sync list
    const isDomainSynced = config.syncDomains.some(domain => {
      const cookieDomain = cookie.domain.startsWith('.') ? cookie.domain.slice(1) : cookie.domain;
      return cookieDomain === domain || domain.endsWith(cookieDomain);
    });

    if (isDomainSynced) {
      // Debounce: wait 1 second to batch rapid changes
      await debounceSync();
    }
  } catch (error) {
...
async function debounceSync() {
  if (syncTimeout) clearTimeout(syncTimeout);
  syncTimeout = setTimeout(async () => {
    await sendWebhookSync();
  }, 1000);
}
