...
## File Transfer (tg_file_transfer.py)

Large file transfer via Telegram — download, upload, split, assemble.

```bash
cd ~/.agent-reach/tools/telegram-toolkit

# Download file from message
python3 tg_file_transfer.py download @channel/123 --output /tmp/
python3 tg_file_transfer.py download @channel/123 --resume  # resume partial

# Upload file
python3 tg_file_transfer.py upload ~/bigfile.zip --to @channel
python3 tg_file_transfer.py upload ~/bigfile.zip --to @user --caption "sent file"

# Split large file before upload
python3 tg_file_transfer.py split ~/huge.iso --chunk-size 1.5G
# → outputs .chunks/ directory with .partNNNN files + metadata

# Assemble chunks back
python3 tg_file_transfer.py assemble /path/to/.chunks/

# Check file info + Telegram size compatibility
python3 tg_file_transfer.py info ~/bigfile.zip
```
...
|--------|-------|
| Bot API | 50 MB |
| MTProto (regular) | 2 GB |
| MTProto (premium) | 4 GB |

Files >2GB need `split` before upload. Split produces `.partNNNN` files + `.tg_chunk_meta.json` with MD5 checksums for verification.

**Pitfall — resume:** `--resume` flag re-downloads from `.part` partial file. Without it, partial files are deleted on retry.

**Pitfall — force upload:** `--force` flag bypasses the 2GB check. May work for premium accounts but will fail for regular.
